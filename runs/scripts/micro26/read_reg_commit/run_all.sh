#!/usr/bin/env bash
#
# Run ChampSim (commit-time register-read counter on read_reg_commit branch) on
# traces under traces/google_traces and/or traces/spec17 with 0 warmup and 50M
# simulation instructions by default. Traces packed in *.zip archives are
# extracted first. Individual runs are launched in parallel.
#
# Logs go to runs/output/micro26/read_reg_commit/cs_logs/ (separate from read_reg/).
#
# Usage:
...
RESULTS_DIR="${CHAMPSIM_ROOT}/runs/output/micro26/read_reg_commit/cs_logs"
...
Commit-time register-read histogram (read_reg_commit branch binary).
...
Environment variables:
  ...
  SIM            Simulation instructions (default: 50000000)
...
main() {
  ...
  echo "Counter:        commit-time (read_reg_commit)"
