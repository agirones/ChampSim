...
Re-run simulations with a rebuilt read_reg_commit binary after changing the counter.

Output: runs/output/micro26/read_reg_commit/graphs/zero_read_producers.tex
"""
...
OUTPUT_TEX = ROOT / "runs/output/micro26/read_reg_commit/graphs/zero_read_producers.tex"
...
def collect_data(log_dir: Path) -> dict[str, dict[str, float]]:
    ...
    for log_file in iter_log_files(log_dir):
        ...
        if counts is None:
            print(f"  WARNING: no ZERO_READ_PROD lines in {log_file} (rebuild read_reg_commit and re-run)")
            continue
...
def generate_tikz(data: dict[str, dict[str, float]], categories: list[str], category_labels: list[str], output_path: Path) -> None:
    ...
    tex = (
        ...
        r"    ylabel          = {Fraction of Zero-Read Lifetimes (commit)}," "\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot GPR zero-read register lifetimes (commit-time counter).")
    ...
    if not raw_data:
        print("No zero-read producer data found. Rebuild bin/champsim on read_reg_commit and re-run the batch script.")
        return
